...
<template>
  <Card class="my-3 xl:w-[547px] w-full">
    <template #content>
      <div class="md:grid md:grid-cols-4">
        <div class="rounded-l-lg col-span-1 bg-[url('../assets/atelier.png')] bg-cover"></div>
        <div class="col-span-3 text-left p-4">
          <ul>
            <li class="font-semibold">{{ props.data.nom }} ({{ props.data.millesime }}) - {{ props.data.volume }}cl</li>
          ...
          <div class="xl:flex justify-between mt-3">
            ...
            <div class="xl:flex block justify-end mr-2">
              <a @click="prepareForModification"
                 class="text-center bg-[#2A8DA3] block text-white px-3 py-2 m-1 rounded-md cursor-pointer">Modifier</a>
              <a @click="prepareForDeletion"
                 class="text-center bg-[#E04242] block text-white px-3 py-2 m-1 rounded-md cursor-pointer">Supprimer</a>
            </div>
          </div>
        </div>
      </div>
