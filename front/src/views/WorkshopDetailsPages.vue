    ...
    <div class="relative min-h-screen bg-no-repeat bg-cover bg-fixed">
        ...
        <div class="absolute inset-0 flex items-center justify-center" style="height: 470px;">
            <div class="text-center text-white">
                <h1 class="text-3xl md:text-4xl font-bold">{{ atelier.thematique }}</h1>
                <p class="text-sm md:text-lg mt-2">{{ atelier.nom }}.</p>
            </div>
        </div>
                    ...
                    <div class="md:flex-1 border-l-2 border-lite-grey pl-4">
                        ...
                        <ul class="list-none pl-5 mb-4" v-for="session in atelier.ateliers" :key="session.id">
                            <li class="text-red">
                                ...
                                <span class="text-dark-red pl-2">{{ new Date(session.date_debut).toLocaleDateString() }} à {{ new Date(session.date_debut).toLocaleTimeString() }}</span>
                            </li>
                            <li class="text-red">
